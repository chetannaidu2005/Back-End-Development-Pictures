...
def create_picture():
    ...
    existing = next((pic for pic in data if pic.get("id") == new_picture.get("id")), None)
    if existing:
...
def update_picture(id):
    ...
    for idx, pic in enumerate(data):
        if pic.get("id") == id:
            data[idx] = updated_picture if updated_picture else pic
            return jsonify(data[idx]), 201
